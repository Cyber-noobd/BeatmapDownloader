...
class WelcomeScreen(Screen):
        ...
        with Container(id='outer'):
            ...
            with Container(id="main"):
                ...
                with Container(id="welcomearea"):
                    ...
                    yield Label("", id="version", classes="text")
    ...
    def getLatestVersion(self):
        ...
        try:
            latest = json.loads(self.app.r.client.get(url).content)[0].get('name')
            if localVersion == latest:
                vl.update(f"当前版本:{localVersion} 最新版本:{latest}\n")
            else:
                vl.update(f"当前版本:{localVersion} 最新版本:{latest}\n请前往Github项目页获取更新(链接在下方)\n")
        except Exception as e:
            vl.update(f"当前版本:{localVersion} 最新版本:获取失败!请检查网络连接。\n")
            log(e)
